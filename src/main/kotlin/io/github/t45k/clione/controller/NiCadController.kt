...
class NiCadController(private val sourceCodePath: Path, private val config: RunningConfig) : CloneDetectorController {
    ...
    override fun executeOnNewRevision(changedFiles: Set<String>): Pair<CloneSets, IdCloneMap> {
        ...
        execute()
        val (cloneSets: CloneSets, idCloneMap: MutableMap<Int, CloneInstance>) = parseResultXML(changedFiles, CloneStatus.DELETE)
    ...
    override fun executeOnOldRevision(changedFiles: Set<String>): Pair<CloneSets, IdCloneMap> {
        ...
        execute()
        val result: Pair<CloneSets, IdCloneMap> = parseResultXML(changedFiles, CloneStatus.ADD)
        deleteNiCadResultFiles()
        logger.info("[END]\tclone detection on old revision")
        return result
    }
    ...
    private fun parseCandidateXML(): List<CloneInstance> =
    ...
    private fun parseResultXML(changedFiles: Set<String>, initialStatus: CloneStatus): Pair<CloneSets, MutableMap<Int, CloneInstance>> {
        val cloneRelationGraph: Graph<Int, DefaultEdge> = SimpleGraph(DefaultEdge::class.java)
        val idCloneMap: MutableMap<Int, CloneInstance> = mutableMapOf()
        ...
        DocumentBuilderFactory.newInstance()
            ...
            .map { clonePair ->
                ...
                convertXmlElementToCloneInstance(nodeList.item(0) as Element, initialStatus, changedFiles) to
                    convertXmlElementToCloneInstance(nodeList.item(1) as Element, initialStatus, changedFiles)
            }
            .forEach {
                cloneRelationGraph.addVertex(it.first.id)
                cloneRelationGraph.addVertex(it.second.id)
                cloneRelationGraph.addEdge(it.first.id, it.second.id)

                idCloneMap[it.first.id] = it.first
                idCloneMap[it.second.id] = it.second
            }

        return BronKerboschCliqueFinder(cloneRelationGraph)
            .iterator()
            .asSequence()
            .toList() to idCloneMap
    }
    ...
    private fun convertXmlElementToCloneInstance(element: Element, initialStatus: CloneStatus, changedFiles: Set<String>): CloneInstance {
        ...
        return if (changedFiles.contains(fileName)) {
            ...
            CloneInstance(fileName, startLine, endLine, id, CloneStatus.STABLE, null)
        }
    }
