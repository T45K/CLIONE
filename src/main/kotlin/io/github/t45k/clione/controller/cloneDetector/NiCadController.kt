...
class NiCadController(sourceCodePath: Path, config: RunningConfig) : AbstractCloneDetectorController(sourceCodePath, config) {
    ...
    private fun detectClones() {
        ...
        val configFilePath: Path = generateCGI()
        val command: Array<String> = arrayOf("./nicad6", granularity, lang, sourceCodePath.toString(), configFilePath.toString())
    ...
    private fun generateCGI(): Path {
        val config: String = """
            ...
            exclude=""
        """.trimIndent()
        return Files.writeString(sourceCodePath.resolve("clione.cgi"), config).toRealPath()
    }
    ...
    override fun cleanup() =
        Files.list(sourceCodePath.parent)
            .filter {
                it.fileName.toString().startsWith("${sourceCodePath.fileName}_") ||
                    it.fileName.toString() == "clione.cgi"
            }
