...
class CloneTracker(
    ...
    fun track(): Pair<List<List<CloneInstance>>, List<List<CloneInstance>>> {
        ...
        return filterInconsistentChange(oldCloneSets, oldIdCloneMap) to filterInconsistentChange(
            newCloneSets,
            newIdCloneMap
        )
    }
    ...
    fun filterInconsistentChange(cloneSets: CloneSets, idCloneMap: IdCloneMap): List<List<CloneInstance>> =
        cloneSets.filterNot { cloneSet ->
            ...
            .filter { cloneSet ->
                cloneSet.all {
                    (idCloneMap[it] ?: error("")).status == CloneStatus.STABLE ||
                        (idCloneMap[it] ?: error("")).status == CloneStatus.MODIFY
                }
            }
