...
import io.github.t45k.clione.core.CloneDetector.SOURCERERCC
...
class ClioneGsonMethod {
    ...
    fun test() {
        ...
        val dir = if (Files.notExists(resDir.resolve("gson_method"))) {
            Files.createDirectory(resDir.resolve("gson_method"))
        } else {
            resDir.resolve("gson_method")
        }
...
652 2cac11b44984640ffa2ec5cccae0c8643b5bce92 6e57df7e9648f4a48b500ac9a181a058829bcee1""".trimMargin().split("\n")
            .forEach { line ->
                ...
                GitController.cloneIfNotExists(repositoryFullName, "", pullRequest).use { git ->
                    val config = RunningConfig("gson/src/main", cloneDetector = SOURCERERCC, granularity = METHOD)
